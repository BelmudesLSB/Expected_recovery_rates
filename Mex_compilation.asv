...
% Common parameters:
params.b_grid_size = 252;                % Number of points in the grid for the bond price.
...
figure;
hold on;
plot(B_grid, V_benchmark(y_choice, :),'black', 'LineWidth', 2);
hold on;
plot(B_grid, V_high(y_choice, :), 'b', 'LineWidth', 2);
% Plot for Alpha Low Calibration
...
legend({['$\alpha_{base} = ' num2str(params_benchmark.alpha) '$'], ['$\alpha_{high} = ' num2str(params_high.alpha) '$'], ['$\alpha_{low} = ' num2str(params_low.alpha) '$']}, ...
